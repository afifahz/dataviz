...
dim(iris) # melihat jumlah observasi dan jumlah variabel, mengetahui dimensi / jumlah baris dan kolom
str(iris) # mencetak apa saja kolomny, isi dan tipe nya 
colnames(iris) # melihat nama kolom di dataset iris
head(iris) # mencetak 6 data teratas di dataset iris
tail(iris) # mencetak 6 data terakhir di dataset iris
summary(iris) # melihat nilai statistik dasar dari data (nilai min dan max, rata2, median, quadran)
...
length(iris$Petal.Width) # mengetahui jumalah data dr suatu kolom
levels(iris$Species) # mengetahui jumlah level dr variabl
length(levels(iris$Species)) # 
unique(iris$Species) # mengetahui jumlah data unik
...
vis_dat(typical_data) # summary dalam bentuk visual
skim(typical_data) # cetak di konsol, kelompok berdasar tipe data
```
...
1. mengambil data tahun, bulan, kecepatan angin, dan tekanan udara dari dataset storms
2. melakukan filter data dari tahun 2000 s.d 2015
3. mengubah bulan dari angka menjadi nama bulan
4. melakukan grouping berdasarkan bulan
5. menghitung rata2 kecepatan angin dan tekanan udara
